...
currentAccount: any;
    ...
    loadAll() {
        this.operationLogService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/operation-log'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/operation-log', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OperationLog) {
        return item.id;
    }
    registerChangeInOperationLogs() {
        this.eventSubscriber = this.eventManager.subscribe('operationLogListModification', (response) => this.loadAll());
    }
