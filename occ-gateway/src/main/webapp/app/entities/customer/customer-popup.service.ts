        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.customerService.find(id).subscribe((customer) => {
                    customer.ts = this.datePipe
                        .transform(customer.ts, 'yyyy-MM-ddTHH:mm:ss');
                    customer.timeCreated = this.datePipe
                        .transform(customer.timeCreated, 'yyyy-MM-ddTHH:mm:ss');
                    customer.timeModified = this.datePipe
                        .transform(customer.timeModified, 'yyyy-MM-ddTHH:mm:ss');
