...
import com.yonyou.occ.ms.order.command.handler.PurchaseOrderCommandHandler;
...
public class AxonConfiguration {
    ...
    public Snapshotter snapshotter() {
        return new AggregateSnapshotter(eventStore, purchaseOrderAggregateFactory());
    }
    ...
    public PurchaseOrderAggregate purchaseOrderAggregate() {
        return new PurchaseOrderAggregate();
    }

    @Bean
    public AggregateFactory<PurchaseOrderAggregate> purchaseOrderAggregateFactory() {
        SpringPrototypeAggregateFactory<PurchaseOrderAggregate> aggregateFactory =
            new SpringPrototypeAggregateFactory<>();
        aggregateFactory.setPrototypeBeanName("purchaseOrderAggregate");
        return aggregateFactory;
    }

    @Bean
    public Repository<PurchaseOrderAggregate> purchaseOrderAggregateRepository() {
        EventCountSnapshotTriggerDefinition snapshotTriggerDefinition = new EventCountSnapshotTriggerDefinition(
            snapshotter(), 10);
        return new EventSourcingRepository<>(purchaseOrderAggregateFactory(), eventStore, snapshotTriggerDefinition);
    }

    @Bean
    public PurchaseOrderCommandHandler purchaseOrderCommandHandler() {
        return new PurchaseOrderCommandHandler(purchaseOrderAggregateRepository());
    }
}
